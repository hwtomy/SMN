...
class GroupGates(nn.Module):
    def __init__(self, groups=((0,1),(2,3),(4,5)),
                 init_logit=(-1,0, -1.5, -2.5),
                 temperature=1.0,
                 learnable=True):
        super().__init__()
        self.groups = groups
        self.temperature = temperature
        p = torch.tensor(init_logit, dtype=torch.float32)
        self.logit = nn.Parameter(p, requires_grad=learnable)

    def forward(self, K:int):
        g = torch.zeros(K, device=self.logit.device)
        probs = torch.sigmoid(self.logit / self.temperature)  
        for gi, idxs in enumerate(self.groups):
            g[list(idxs)] = probs[gi]
        return g 


"""
The network of SMN. If for 3D, the class INRN is recommended which is the same as this class
...
class FourierFeatureMapping(nn.Module):
    def __init__(
        ...
        max_f = float(num_freqs - 1) if max_freq_log2 is None else float(max_freq_log2)
        if log_sampling:
            freq_bands = 2.0 ** torch.linspace(0.0, max_f, num_freqs)
        else:
            freq_bands = torch.linspace(1.0, 2.0**max_f, num_freqs)

        self.register_buffer("freq_bands", freq_bands)


        self.out_dim = (in_dim if include_input else 0) + in_dim * 2 * num_freqs

    def forward(self, x: torch.Tensor) -> torch.Tensor:


        orig_shape = list(x.shape[:-1])
        x_flat = x.view(-1, self.in_dim)      # [B, in_dim]
        ...
        for freq in self.freq_bands:          # freq 标量
            outs.append(torch.sin(x_flat * freq))
            outs.append(torch.cos(x_flat * freq))

  
        encoded = torch.cat(outs, dim=-1)     # [B, out_dim]
        encoded = encoded.view(*orig_shape, self.out_dim)
        return encoded
...
class ModMLP(nn.Module):
    ...
    def __init__(self, in_features, out_features, hidden_features, n_hidden_layers,
        ...
        if first_layer_mode == "positional_encoding":
            self.layers.append(FourierFeatureMapping(
                                        in_dim=6,           
                                        num_freqs=10,       
                                        max_freq_log2=9,    
                                        include_input=True,  
    ...
    def __init__(self, in_features, out_features, hidden_features, n_hidden_layers,
        ...
        if first_layer_mode == "positional_encoding":
            ...
            self.layers.append(FFM(in_features, out_features))
                ...
                # self.pe = PE1(input_dim=in_features, multires=12, include_input=True)
        ...
        for i in range(n_hidden_layers):
            if inr_type == "wire":
                ...
                if i==0 :
                    self.layers.append( ModLinear(
                        hidden_features, hidden_features,
                        init_mode="uniform", nonlinearity="relu", mod_type=mod_type, omega_0=1., ca=False))
                else:
                    self.layers.append( ModLinear(
                        hidden_features, hidden_features,
                        init_mode="uniform", nonlinearity="relu", mod_type=mod_type, omega_0=1.))
        ...
        if mask:
            for i in range(3):
                self.masks.append( ModLinear(
                    in_features, in_features,bias = False,
                    init_mode="uniform", nonlinearity="relu", mod_type=mod_type, omega_0=1.))
            self.masks = nn.ModuleList(self.masks)
